...
bool isBalanced(string s)
{
    ...
    for (int i = 0; i < s.length(); i++)
    {
        ...
        {
            // Xet cac truong hop co phai la chu so bat dau cua 1 operand khong
            if (i - 1 >= 0) 
            {
...
bool isValidSpace(string s)
{
    ...
    for (i = 0; s[i] != '\0'; i++)
    {
        if (s[i] == ' ' && ((i + 1 < n && s[i + 1] == ' ') || (i - 1 >= 0 && s[i - 1] == ' '))) // check truong hop 2 dau space canh nhau
        {
            return false;
        }
        ...
        if (temp)
        {
            // Xet truong hop dau space bi sai cho phan tu la ngoac dong hoac ngoac mo
        ...
        if (isOperator(s[i]))
        {
            // Xet 2 phan tu ke canh cua 1 operator
            // Do bieu thuc la 1 infix expression, nen phai co it nhat 1 dau space ben canh cua operator
            if (i + 1 < n && s[i + 1] != ' ')
            {
                return false;
            }
...
bool checkString(string s)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        {
            // Xet 2 phan tu truoc va sau operator co phai toan hang khong
            if (i - 2 < 0 || i + 2 > len)
                return false;
